...
export async function GET(request: Request) {
  // Seguridad: vale el token manual (?secret=...) O la cabecera del cron de Vercel.
  // Allowed: manual token (?secret=...) OR Vercel cron's Authorization header.
  ...
  try {
    ...
    // 2) Standings -> equipos + grupo
    const standings = await apiGet(`/standings?league=${LEAGUE}&season=${SEASON}`);
    const groupsData: Array<Array<Record<string, unknown>>> =
      standings[0]?.league?.standings ?? [];
    ...
    // 3) Standings rows (clasificación real)
    ...
    // 4) Fixtures -> matches (solo fase de grupos)
    ...
    for (const fx of fixtures as Array<Record<string, unknown>>) {
      ...
      const fixture = fx.fixture as {
        id: number; date: string; venue?: { name?: string }; status?: { short?: string };
      };
    ...
    await supabase.from("api_sync_logs").insert({
      source: "api-football",
      endpoint: "/standings + /fixtures",
      status: "ok",
      rows_upserted: summary.teams + summary.matches,
