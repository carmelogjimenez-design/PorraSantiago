...
export default async function PronosticosPage() {
  ...
  const [profileRes, myPtsRes, groupsRes, teamsRes, matchesRes, predsRes] = await Promise.all([
    ...
    supabase.from("matches").select("id,group_id,home_team_id,away_team_id,kickoff_at,status,home_score,away_score").order("kickoff_at"),
  ...
  const matches: MatchVM[] = matchesRaw.map((m) => {
    ...
    return {
      id: m.id,
      group: m.group_id != null ? (labelById.get(m.group_id) ?? "") : "",
