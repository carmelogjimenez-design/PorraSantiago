...
export async function savePrediction(_prev: PredState, formData: FormData): Promise<PredState> {
  ...
  if (!match_id || Number.isNaN(pred_home) || Number.isNaN(pred_away) || pred_home < 0 || pred_away < 0)
  ...
  if (error) {
    ...
    if (msg.includes("lock") || msg.includes("bloque") || msg.includes("kickoff"))
      return { error: "El partido ya empezó / Match locked" };
