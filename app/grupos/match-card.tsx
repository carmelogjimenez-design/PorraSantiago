...
function Flag({ src, name }: { src: string | null; name: string }) {
  if (!src) {
    return (
      <span className="grid h-7 w-9 flex-none place-items-center rounded bg-[var(--bg-soft)] text-[10px] font-bold text-[var(--text-dim)]">
  ...
  return (
    <img
      src={src}
      alt={name}
      className="h-7 w-9 flex-none rounded object-cover ring-1 ring-[var(--border)]"
    />
  );
}
...
export default function MatchCard({ m }: { m: MatchVM }) {
  ...
  const open =
    m.status === "scheduled" && kickoff.getTime() > Date.now();
  ...
  return (
    <div
      className={`card p-3.5 ${live ? "border-[var(--accent)]" : ""}`}
    >
      <div className="mb-3 flex items-center justify-between text-[11px] text-[var(--text-dim)]">
        {live ? (
          <span className="flex items-center gap-1.5 font-extrabold uppercase tracking-wide text-[var(--accent)]">
            <span className="blink h-[7px] w-[7px] rounded-full bg-[var(--accent)]" />
            En directo
          </span>
        ) : (
      ...
      {open ? (
        <form action={action}>
          ...
          <div className="mt-3 flex items-center justify-between border-t border-[var(--border)] pt-2.5">
            <span className="text-xs">
              {state?.error ? (
                <span className="font-bold text-[var(--accent-deep)]">
                  {state.error}
                </span>
              ) : justSaved ? (
                <span className="pop font-extrabold text-[var(--green)]">
                  ✓ Guardado
                </span>
              ) : m.predHome != null ? (
                <span className="text-[var(--text-dim)]">
                  Pronóstico: {m.predHome}-{m.predAway}
                </span>
              ) : (
        ...
        <>
          ...
          <div className="mt-3 flex items-center justify-between border-t border-[var(--border)] pt-2.5 text-xs">
            ...
            {live && m.predHome != null && (
              <span className="rounded-full bg-[var(--accent-soft)] px-2.5 py-1 font-extrabold text-[var(--accent-deep)]">
                en juego
              </span>
            )}
          </div>
